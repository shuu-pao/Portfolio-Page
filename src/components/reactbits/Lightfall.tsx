...
const prepColors = (input?: string[]): { arr: [number, number, number][]; count: number; avg: [number, number, number] } => {
  const base = (input && input.length ? input : ['#A6C8FF', '#5227FF', '#FF9FC']).slice(0, MAX_COLORS);
...
const Lightfall = (props: LightfallProps) => {
  const {
    className = "",
    dpr,
    paused = false,
    colors = ['#A6C8FF', '#5227FF', '#FF9FC'],
  ...
  const programRef = useRef<import('ogl').Program>(null);
  const meshRef = useRef<import('ogl').Mesh<import('ogl').Geometry, import('ogl').Program>>(null);
  const geometryRef = useRef<import('ogl').Triangle>(null);
