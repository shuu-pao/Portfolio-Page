...
**Superseded by Task 4** (`docs/superpowers/plans/2026-08-02-hero-mees-redesign.md`): §2.1's `4 / 5` portrait image sizing and §4's "no absolute overlay elements" / separate ~30%-width text column description below were both eyeballed incorrectly. Task 4 replaces them with values derived from pixel-measuring `mees-hero-top.png` directly: the photo is ~32% of viewport width (near-square, not portrait), and the "P./" label, cursive, and bio text are a column that deliberately overlaps the photo's right edge before continuing onto the open background — not a separate non-overlapping column. Treat Task 4's plan text as authoritative over the layout specifics below; the rest of this spec (color, typography, accessibility, marquee behavior) is unaffected.
